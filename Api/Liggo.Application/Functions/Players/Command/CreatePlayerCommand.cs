...
namespace Liggo.Application.Functions.Players.Command
{
    ...
    public class CreatePlayerCommandHandler : IRequestHandler<CreatePlayerCommand, string>
    {
        private readonly IPlayerRepository _playerRepository;
        private readonly ICurrentUserService _currentUserService;
        private readonly ISchoolRepository _schoolRepository;
        
        public CreatePlayerCommandHandler(IPlayerRepository playerRepository, ICurrentUserService currentUserService, ISchoolRepository schoolRepository)
        {
        ...
        public async Task<string> Handle(CreatePlayerCommand request, CancellationToken cancellationToken)
        {
            ...
            bool hasReachedLimit = await _schoolRepository.HasReachedPlayerLimitAsync(Guid.Parse(secureSchoolId), 30, cancellationToken);

            if (hasReachedLimit)
            {
                throw new Exception("Has alcanzado el límite de jugadores. Selecciona otro equipo.");
            }
            ...
            var newPlayer = new Player
            {
                ...
                Info = new PlayerInfo
                {
                    Name = request.Name, Dob = request.Dob, Gender = request.Gender,
                    Position = request.Position, Weight = request.Weight, Height = request.Height
                }
            };
